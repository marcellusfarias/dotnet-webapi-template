...
_Tracking_ vs _no-tracking_: this [Microsoft's article](https://learn.microsoft.com/en-us/ef/core/querying/tracking) is very informative. In a nutshell, _tracking_ persists changes to the entity in the SaveChanges method and can improve performance if an entity is already in the context (meaning one less trip to the database). _No-tracking_ doesn't keep state, so it uses less memory and is faster in general, being great for _readonly_ operations. Furthermore, there is still _No-Tracking with Identity Resolution_ ([check example](https://macoratti.net/22/05/ef_asnoidresol1.htm)). It's good for relationship "one-to-many" for memory optimization, since it keep the repeated entity in the context. The default on this project is to track; however, for readonly operations, we tend to use no-tracking and no-tracking with identity resolution.
